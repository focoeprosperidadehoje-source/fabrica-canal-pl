#!/usr/bin/env python3
"""
ao_vivo_pl.py — Live 24/7 Canal PL — Arquitetura Pivot
Voz: pl-PL-ZofiaNeural (Matka Boża Częstochowska)
...
Blocos base (~20min H) chegam via rsync do GitHub Actions 6×/dia.
...
LOG_FILE = Path("/root/ao_vivo_pl/ao_vivo.log")
...
log = logging.getLogger("ao_vivo_pl")
...
CANAL_ID       = os.environ.get("CANAL_ID_PL", "UCEIh4A01a8tBI1XWp-p6Kkw")
PLAYLIST_LIVES = os.environ.get("PLAYLIST_ID_LIVES_PL", "PLadQnfhLGt7k")
FUSO           = pytz.timezone(os.environ.get("FUSO_PL", "Europe/Warsaw"))

STREAM_KEY_H   = os.environ.get("STREAM_KEY_H_PL", "")
STREAM_KEY_V   = ""   # Live PL começa apenas horizontal
INGEST_URL     = os.environ.get("INGEST_URL", "rtmp://a.rtmp.youtube.com/live2")
BROADCAST_ID_H = os.environ.get("BROADCAST_ID_H_PL", "")
MODO_PERMANENTE = bool(STREAM_KEY_H)

BASE_DIR        = Path("/root/ao_vivo_pl")
DIR_BLOCOS      = BASE_DIR / "blocos"
DIR_SUPLICAS    = BASE_DIR / "suplicas"
DIR_INSUMOS_H   = BASE_DIR / "insumos_h"   # imagens Częstochowa 16:9
DIR_INSUMOS_V   = BASE_DIR / "insumos_v"   # fallback
DIR_MUSICAS_M   = BASE_DIR / "musicas" / "manha"
DIR_MUSICAS_N   = BASE_DIR / "musicas" / "noite"
DIR_VIDEOS_BASE = BASE_DIR / "videos_base"  # slides Częstochowa sem som (10min cada)

PLAYLIST_H_FILE = BASE_DIR / "playlist_h.txt"
YT_TOKEN_FILE   = BASE_DIR / "youtube_token.json"
GCP_CREDS_FILE  = BASE_DIR / "google_credentials_pl.json"

# IDs Google Drive — assets para súplicas no VPS
DRIVE_INSUMOS_H_ID = os.environ.get("DRIVE_CZESTOCHOWA_H_ID", "")
DRIVE_MUSICAS_M_ID = os.environ.get("DRIVE_MUSICAS_M_ID_PL", "")
DRIVE_MUSICAS_N_ID = os.environ.get("DRIVE_MUSICAS_N_ID_PL", "")
...
DURACAO_CICLO_SEG    = 12 * 3600     # 12h por broadcast
BLOCOS_MINIMOS       = 1
...
CHAVES_CHAT = [c for c in [
    os.environ.get("GEMINI_KEY_LIVE_CHAT_1_PL", ""),
    os.environ.get("GEMINI_KEY_LIVE_CHAT_2_PL", ""),
    os.environ.get("GEMINI_KEY_LIVE_CHAT_3_PL", ""),
] if c]
...
_THUMB_LINHAS = {
    "manha": ["MOC I", "UZDROWIENIE", "Z MATKĄ BOŻĄ"],
    "tarde":  ["BOŻA", "OCHRONA", "Z MATKĄ BOŻĄ"],
    "noite":  ["ODPOCZYNEK", "I UZDROWIENIE", "Z MATKĄ BOŻĄ"],
}

TITULOS_LIVE = {
    0: "🔴 Matka Boża Częstochowska CHRONI Cię TERAZ przed każdym atakiem — Potężna modlitwa NA ŻYWO",
    1: "🔴 Matka Boża Częstochowska przełamuje DZIŚ to zniewolenie — Uwolnienie NA ŻYWO",
    2: "🔴 Matka Boża Częstochowska ODBUDOWUJE Twoją Rodzinę DZIŚ — Cud Pojednania NA ŻYWO",
    3: "🔴 Matka Boża Częstochowska OTWIERA DZIŚ Zamknięte Drzwi — Cud Opatrzności NA ŻYWO",
    4: "🔴 Matka Boża Częstochowska UZDRAWIA Twoje Ciało TERAZ — Cud Uzdrowienia NA ŻYWO",
    5: "🔴 Cudowny Płaszcz Matki Bożej Okrywa Cię TERAZ — Ochrona i Cuda NA ŻYWO",
    6: "🔴 Matka Boża Częstochowska Ma DLA CIEBIE CUD DZIŚ — Otrzymaj NA ŻYWO",
}
...
def _garantir_playlists_tematicas(yt) -> dict:
    ...
    for weekday, nome in _NOMES_PLAYLIST_TEMATICA.items():
        if nome in existentes:
            ...
            log.info(f"Playlista tematyczna znaleziona: {nome} → {existentes[nome]}")
        else:
            try:
                r = yt.playlists().insert(
                    ...
                    body={
                        "snippet": {
                            "title": nome,
                            "description": f"Transmisje modlitwy i VOD — {nome}",
                            "defaultLanguage": "pl",
                        },
                ...
                log.info(f"Playlista tematyczna UTWORZONA: {nome} → {r['id']}")
            except Exception as e:
                log.warning(f"Utwórz playlistę '{nome}': {e}")
...
def _renomear_vod_e_classificar(bid: str):
    ...
    weekday = now.weekday()
    titulo_base = re.sub(r"[🔴🟢🔵]|— Potężna modlitwa|\s*NA ŻYWO", "", TITULOS_LIVE[weekday]).strip()
    ...
    try:
        ...
        log.info(f"VOD {bid} przemianowany: {titulo_vod}")
    except Exception as e:
        log.warning(f"przemianuj VOD {bid}: {e}")
    try:
        ...
        if pid:
            ...
            log.info(f"VOD {bid} → playlista tematyczna weekday={weekday}: {pid}")
    except Exception as e:
        log.warning(f"playlista tematyczna VOD {bid}: {e}")
...
def _load_gcp_info() -> dict:
    ...
    return json.loads(os.environ["GOOGLE_CREDENTIALS_PL"])
...
def _creds_youtube() -> OAuthCredentials:
    raw = os.environ.get("YOUTUBE_TOKEN_PL") or YT_TOKEN_FILE.read_text()
    data = json.loads(raw)
    ...
    if creds.expired and creds.refresh_token:
        ...
        log.info("YouTube token PL renovado.")
    return creds
...
def garantir_assets_vps():
    try:
        ...
        if len(imgs_h) < 5:
            log.info("Baixando imagens Częstochowa horizontal...")
...
def extrair_suplicantes(msgs: list[dict], max_s: int = 6) -> list[dict]:
    palavras = ["módl", "prosz", "potrzebu", "chory", "praca", "rodzin", "małżeństwo",
                "uzdrow", "cud", "pomoc", "ból", "nałóg", "Matka Boża", "wstaw"]
    resultado = []
    for m in msgs:
...
def _gerar_roteiro_suplica(suplicantes: list[dict]) -> str:
    ...
    prompt = (
        f"Jesteś Matką Bożą Częstochowską, Królową Polski, mówiącą w pierwszej osobie, po polsku.\n"
        f"Wstawiaj się za tymi duszami przez 2-3 minuty.\n\n"
        f"Intencje wiernych:\n{linhas}\n\n"
        f"Instrukcje:\n"
        f"- WSPOMNIJ każdą osobę po imieniu z jej konkretną intencją\n"
        f"- Ton macierzyński i ciepły, 380-450 słów\n"
        f"- Uwzględnij krótkie błogosławieństwo na końcu\n"
        f"- Tylko ciągły tekst, bez markdown, bez tytułów\n"
        f"- Ostatnie zdanie jest składniowo niekompletne, aby płynnie połączyć się\n"
        f"  z następną modlitwą w transmisji"
    )
...
def _stream_id_da_chave(yt) -> str:
    ...
    sid_env = os.environ.get("STREAM_ID_H_PL", "")
    if sid_env:
    ...
    if len(itens) == 1:
        sid = itens[0]["id"]
        log.warning(f"STREAM_KEY_H_PL não achada por nome — usando único liveStream ({sid})")
        _stream_id_cache["id"] = sid
        return sid
    raise RuntimeError(
        f"liveStream da STREAM_KEY_H_PL não encontrado ({len(itens)} streams) "
        "— defina STREAM_ID_H_PL no .env"
    )
...
def criar_broadcast_permanente(yt) -> str:
    ...
    try:
        ...
        snip["defaultLanguage"]      = "pl"
        snip["defaultAudioLanguage"] = "pl"
        yt.videos().update(part="snippet", body={"id": bid, "snippet": snip}).execute()
    except Exception as e:
        log.warning(f"idioma do broadcast {bid}: {e}")
    log.info(f"Broadcast PL criado (não listado, autoStart): {bid} — {titulo}")
    return bid
...
def _aplicar_thumbnail(yt, bid: str):
    ...
    try:
        ...
        brand = "Matka Boża Częstochowska · Modlitwa 24 Godziny"
        f_brand = _fonte(30)
        ...
        log.info(f"Thumbnail PL aplicada ao broadcast {bid}")
        os.remove(tmp)
    except Exception as e:
        log.warning(f"thumbnail PL: {e}")
...
def loop_suplicas():
    ...
    while not _ev_parar.is_set():
        ...
        log.info("Súplicas PL: iniciando geração...")
        try:
            with _lock:
            ...
            if not roteiro:
                log.warning("Súplica PL: roteiro vazio — pulando")
            ...
            if not audio_path.exists() or audio_path.stat().st_size < 1024:
                log.warning("Súplica PL: áudio inválido — pulando")
                _ev_suplica_pronta.set()
                continue
            ...
            _ev_suplica_pronta.set()
            log.info(f"Súplica PL pronta: {sh.name} ({dur}s)")

        except Exception as e:
            log.error(f"Súplica PL: erro: {e}")
            _ev_suplica_pronta.set()
...
def _montar_bloco_h(audio: Path) -> Path:
    ...
    log.info(f"Assembler PL: montando {saida.name} ({dur//60}min)...")
    result = subprocess.run(cmd, capture_output=True, text=True, timeout=4500)
    concat_file.unlink(missing_ok=True)
    if result.returncode != 0:
        saida_tmp.unlink(missing_ok=True)
        raise RuntimeError(f"FFmpeg assembler PL falhou: {result.stderr[-600:]}")
    saida_tmp.rename(saida)
    mb = saida.stat().st_size // (1024 * 1024)
    log.info(f"Assembler PL: {saida.name} pronto ({mb} MB)")
    return saida

def loop_assembler():
    log.info("Assembler PL iniciado — aguardando audio_*.mp3 em blocos/")
    while not _ev_parar.is_set():
        try:
            blocos_h = len(list(DIR_BLOCOS.glob("bloco_*_h.mp4")))
            if blocos_h >= ASSEMBLER_BLOCOS_MAX:
                log.info(f"Assembler PL: cap atingido ({blocos_h}/{ASSEMBLER_BLOCOS_MAX}) — aguardando slot livre")
                now = time.time()
            ...
            for audio in sorted(DIR_BLOCOS.glob("audio_*.mp3")):
                ...
                try:
                    ...
                    log.info(f"Assembler PL: {bloco_h.name} adicionado à rotação.")
                    if bloco_h.exists():
                        audio.unlink(missing_ok=True)
                except Exception as e:
                    log.error(f"Assembler PL erro ({audio.name}): {e}")
        except Exception as e:
            log.error(f"loop_assembler PL erro: {e}")
        _ev_parar.wait(timeout=60)
    log.info("Assembler PL encerrado.")
...
def loop_transmissor():
    ...
    if MODO_PERMANENTE:
        log.info("Modo PERMANENTE PL: usando stream key fixa (sem criar broadcasts via API)")
        ...
        try:
            yt = get_youtube()
            log.info("YouTube API PL OK — broadcast novo a cada ciclo de 12h.")
        except Exception as e:
            log.error(f"YouTube API PL indisponível ({e}) — SEM auto-broadcast!")
        ...
        while not _ev_parar.is_set():
            ...
            while not blocos and not _ev_parar.is_set():
                log.warning("Sem blocos PL disponíveis — aguardando 60s...")
                _ev_parar.wait(timeout=60)
                blocos = listar_blocos()
            if _ev_parar.is_set():
                break
                ...
                try:
                    ...
                    if not bid_h:
                        for _tentativa_bc in range(5):
                            try:
                                ...
                                log.warning(f"criar_broadcast PL: tentativa {_tentativa_bc+1}/5 ERRO: {_ebc}")
                                bid_h = None
                            if bid_h:
                                break
                            log.warning(f"criar_broadcast PL: tentativa {_tentativa_bc+1}/5 sem ID — aguardando 30s")
                            _ev_parar.wait(timeout=30)
                        if not bid_h:
                            log.error("criar_broadcast PL: todas as 5 tentativas falharam — watchdog tentará a cada 2min")
                    else:
                        try:
                    ...
                    if bid_h:
                        ...
                        threading.Thread(target=_publicar_apos_golive, args=(yt, bid_h),
                                         name="PublicaLivePL", daemon=True).start()
                except Exception as e:
                    log.error(f"broadcast PL do ciclo: {e}")
            ...
            ultimo_refresh_rtmp = ciclo_start  # PL: dispara às 12h exatas (EN dispara 11.5h → sempre offset 30min entre eles)

            try:
                while not _ev_parar.is_set():
                    elapsed = time.time() - ciclo_start
                    if elapsed >= DURACAO_CICLO_SEG:
                        log.info(f"Ciclo {ciclo}: 12h completas — encerrando FFmpeg.")
                        break
                    ...
                    if proc_h.poll() is not None:
                        ...
                        if uptime < 30:
                            ...
                            log.warning(f"FFmpeg H PL encerrou em {uptime:.0f}s (rc={rc}) — falha RTMP #{_falhas_rtmp}, aguardando {espera}s")
                            _ev_parar.wait(timeout=espera)
                            if _falhas_rtmp >= 4 and yt and bid_h:
                                log.warning("FFmpeg H PL: 4 falhas RTMP consecutivas — forçando broadcast novo")
                            ...
                            _falhas_rtmp = 0
                            log.warning(f"FFmpeg H PL encerrou (rc={rc}) — reconstruindo e reiniciando")
                    ...
                    if (time.time() - ultimo_refresh_rtmp) >= 12 * 3600:
                        log.info("Refresh RTMP periódico PL: reiniciando FFmpeg H para manter sinal Excelente")
                    ...
                    if not _ev_suplica_gerar.is_set() and (time.time() - ultimo_suplica) >= SUPLICA_INTERVAL:
                        ...
                        if sups_prontas < SUPLICA_MAX_READY:
                            ...
                            log.info(f"Súplicas PL: disparando geração ({sups_prontas} prontas)")
                        else:
                            log.info(f"Súplicas PL: cap atingido ({sups_prontas}/{SUPLICA_MAX_READY}) — skip")
                    ...
                    if _ev_suplica_pronta.is_set():
                        ...
                        if sh and sh.exists():
                            ...
                            log.info(f"Suplica PL inserida | buf_restante={buf_h - elapsed:.0f}s")
                    ...
                    if yt and (time.time() - ultimo_check_bc) >= 120:
                        ...
                        if not bid_h:
                            ...
                            try:
                                ...
                                log.warning(f"Watchdog PL: criar_broadcast falhou: {_ewdg}")
                                bid_h = None
                            if bid_h:
                                with _lock:
                                    _estado["live_id_h"] = bid_h
                            ...
                            for _tentativa in range(3):
                                try:
                                    ...
                                    log.warning(f"watchdog broadcast PL (tentativa {_tentativa+1}/3): {e}")
                                    if _tentativa < 2:
                                        time.sleep(10)
                            if st in ("complete", "revoked"):
                                log.warning(f"Broadcast PL {bid_h} encerrado — criando novo")
                                _finalizar_broadcast(yt, bid_h)
                                try:
                                    bid_h = criar_broadcast_permanente(yt)
                                except Exception as _erv:
                                    log.warning(f"Watchdog PL (revoked): criar_broadcast falhou: {_erv}")
                                    bid_h = None
                                if bid_h:
                                    with _lock:
                                        _estado["live_id_h"] = bid_h
            ...
            if _ev_parar.is_set():
                break
            log.info("Ciclo PL 12h concluído — aguardando 60s para YouTube salvar VOD...")
            if _ev_parar.wait(timeout=60):
                break
            log.info(f"Reiniciando stream PL (ciclo {ciclo + 1})...")
        return
    ...
    while not _ev_parar.is_set():
        ciclo += 1
        log.info(f"══════ CICLO PL {ciclo} ══════")
        blocos = listar_blocos()
        while not blocos:
            log.warning("Aguardando blocos PL do GitHub Actions... retry em 60s")
            if _ev_parar.wait(timeout=60):
                return
            blocos = listar_blocos()
        log.error("MODO DINÂMICO PL não suportado — configure STREAM_KEY_H_PL no .env")
...
def loop_monitor():
    while not _ev_parar.is_set():
        try:
            ...
            log.info(f"MONITOR PL | blocos={len(blocos)} | súplicas_prontas={len(sups)}")

            if len(blocos) < BLOCOS_MINIMOS:
                log.warning(f"ALERTA PL: apenas {len(blocos)} bloco(s) disponíveis.")
...
def main():
    log.info("═══════════════════════════════════════════════════════")
    log.info(" ao_vivo_pl.py — Canal PL — Matka Boża Częstochowska")
    log.info("═══════════════════════════════════════════════════════")
    ...
    log.info("Verificando assets para súplicas PL...")
    garantir_assets_vps()
    ...
    for t in threads:
        t.start()
    log.info("Threads PL iniciadas. Sistema operacional.")

    try:
        while True:
            time.sleep(30)
    except KeyboardInterrupt:
        log.info("Encerrando ao_vivo_pl...")
        _ev_parar.set()
        _ev_suplica_gerar.set()
        with _lock:
        ...
        for t in threads:
            t.join(timeout=15)
        log.info("ao_vivo_pl.py encerrado.")
