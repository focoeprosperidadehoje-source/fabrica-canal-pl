#!/usr/bin/env python3
"""
gerar_videos_base_pl.py — GitHub Actions: cria vídeos slideshow para videos_base/ da live PL

Baixa imagens de Częstochowa do Drive,
aplica blur-fill 1280x720 com a santa CENTRALIZADA,
e encoda vídeos mudos de ~10min (40 imagens × 15s) com Ken Burns + partículas douradas.

Variáveis de ambiente:
  GOOGLE_CREDENTIALS_PL  — JSON da service account
  NUM_VIDEOS             — quantos vídeos criar (padrão: 8)
  DRIVE_FOLDER_ID        — pasta de imagens (padrão: Częstochowa H)
  VIDEO_PREFIX           — prefixo do nome do arquivo de saída (padrão: czestochowa)
"""
...
VIDEO_PREFIX    = os.environ.get("VIDEO_PREFIX") or "czestochowa"
NUM_VIDEOS      = int(os.environ.get("NUM_VIDEOS") or "8")
IMG_POR_VIDEO   = 40
SEG_POR_IMAGEM  = 15
TARGET_W        = 1280
TARGET_H        = 720
DIR_SAIDA       = Path("videos_base_pl")
...
def get_drive():
    raw  = os.environ.get("GOOGLE_CREDENTIALS_PL", "")
    info = json.loads(raw)
...
def listar_imagens(drive, folder_id: str) -> list[dict]:
    if not folder_id:
        print("[WARN] DRIVE_FOLDER_ID não definido — sem imagens do Drive.")
        return []
    items, token = [], None
    while True:
        ...
        if not token:
            break
    print(f"  Imagens na pasta Drive PL: {len(items)}")
    return items
...
def criar_slideshow(imagens: list[Path], saida: Path, overlay: Path | None = None):
    ...
    fps = 30
    frames_por_img = SEG_POR_IMAGEM * fps
...
def main():
    print("=" * 60)
    print(f"gerar_videos_base_pl.py — {VIDEO_PREFIX.upper()} — {NUM_VIDEOS} vídeos")
    print(f"Pasta Drive: {DRIVE_FOLDER_ID or '(não definido)'}")
    print("=" * 60)
    ...
    if not todas:
        print("[ERRO] Nenhuma imagem encontrada na pasta Drive PL.")
        sys.exit(1)
    ...
    print(f"\n{'='*60}")
    print(f"Concluído PL: {criados}/{total_videos} vídeos em {DIR_SAIDA}/")
    if criados == 0:
        sys.exit(1)
